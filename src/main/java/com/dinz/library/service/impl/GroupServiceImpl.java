...
public class GroupServiceImpl implements GroupService {
    ...
    public void updateGroupPermission(Long id, Set<Long> permissionIds) {
        String groupCode = this.findGroupUseForUpdate(id).get("groupCode").toString();
    ...
    public int insert(Group group) {
        Admin admin = new Admin((Long) session.getAttribute("adminId"), session.getAttribute("adminCode").toString());
    ...
    public int delete(Long id) {
        return this.em.createQuery(QueryString.DELETE_GROUP.getQuery())//
                .setParameter("id", id)//
                .setParameter("modifiedBy", this.session.getAttribute("adminCode").toString())//
    ...
    public Map<String, Object> findGroupUseForUpdate(Long id) {
        List<Object[]> groups = this.em.createQuery(QueryString.FIND_GROUP_FOR_UPDATE.getQuery(), Object[].class)//
                .setParameter("id", id).getResultList();
        if (!groups.isEmpty()) {
            Object[] group = groups.get(0);
            Map<String, Object> output = new HashMap<>();
            output.put("id", group[7]);
            output.put("groupCode", group[0]);
            output.put("name", group[1]);
            output.put("description", group[2]);
            output.put("createDate", group[3]);
            output.put("modifiedDate", group[4]);
            output.put("createBy", group[5]);
            output.put("modifiedBy", group[6]);
            try {
                output.put("permissionIds", new JSONParser(group[3].toString()).parseArray());
            } catch (ParseException e) {
                output.put("permissionIds", null);
            }
            return output;
        } else {
            return null;
    ...
    private enum QueryString {
        ...
        DELETE_GROUP("update Group g set g.deleteStatus = 1, g.modifiedBy.adminCode = :modifiedBy, modifiedDate =: modifiedDate where g.id=:id"), //
        FIND_GROUP_FOR_UPDATE(QueryString.FIND_ALL.query + "and g.id = :id"),
        UPDATE_GROUP("update Group g set "//
                + " g.name = :name" //
